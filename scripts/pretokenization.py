...
from utils.train_utils import get_yaml_config
...
def get_args_parser():
    ...
    parser.add_argument('--config', default='configs/training/generator/dar_vqplus.yaml', type=str,
...
def main(args):
    ...
    config = get_yaml_config(config_path=args.config)
    ...
    for samples, target, paths in tqdm(data_loader_train):
        ...
        with torch.no_grad():
            z_quantized, result_dict = tokenizer.encode(samples_all)
            codes = result_dict["min_encoding_indices"].reshape(samples_all.shape[0], -1)
