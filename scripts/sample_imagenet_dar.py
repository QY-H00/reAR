...
Usage:    
torchrun --nnodes=1 --nproc_per_node=8 --rdzv-endpoint=localhost:9999 sample_imagenet_dar.py config=configs/training/generator/dar.yaml \
    experiment.output_dir="dar_b" \
    experiment.generator_checkpoint="dar_b.bin" \
...
from demo_util import get_tokenizer, get_dar_generator
...
def main():
    ...
    generator = get_dar_generator(config)
    tokenizer.to(device)
    generator.to(device)
