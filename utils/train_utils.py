"""Adapted from:
    https://github.com/bytedance/1d-tokenizer/blob/main/utils/train_utils.py
"""
import json
import os
import time
...
def create_dataloader(config, logger, accelerator):
    ...
    if not dataset_config.get("pretokenization", ""):
    ...
    # # potentially, use a pretokenized dataset for ImageNet speed-up.
        ...
        eval_dataloader.num_batches = math.ceil(
            5000 / total_batch_size_without_accum)
...
def get_rear_random_ratio(config, cur_step):
...
def train_one_epoch_generator(
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if model_type == "maskgit":
            ...
            unwrap_model.set_random_ratio(get_rear_random_ratio(config, global_step))
        else:
            raise NotImplementedError
        ...
        if accelerator.sync_gradients:
            ...
            if (global_step + 1) % config.experiment.log_every == 0:
                ...
                logs = {
                    "lr": lr,
                    "lr/generator": lr,
                    "lr/random_ratio": get_rear_random_ratio(config, global_step),
