...
def create_pretrained_tokenizer(config, accelerator=None):
    if config.model.vq_model.finetune_decoder:
        ...
        pretrianed_tokenizer = MaskgitVQ(config.model.vq_model.pretrained_tokenizer_weight)
...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if config.experiment.get("init_weight", ""):
        ...
        if "yucornetto" not in config.experiment.init_weight:
            model_weight = torch.load(config.experiment.init_weight, map_location="cpu")
            if model_cls == "TiTok" and config.model.vq_model.finetune_decoder:
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate([model]):
            ...
            if model_type == "maskgit":
                ...
                loss, loss_dict= loss_module(logits, input_tokens, weights=masks, mask_ratios=mask_ratios)
            elif model_type == "rar":
